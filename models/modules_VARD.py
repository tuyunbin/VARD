...
class NAPE(nn.Module):
    def __init__(self, in_chans=256, embed_dim=256, s=1):
...
class PERL(nn.Module):
    ...
    def forward(self, query_states, key_states, value_states):
        ...
        pos = self.pos(mixed_value_layer, 14, 14)
        context_layer += pos
...
class URD(nn.Module):
    ...
    def forward(self, query_states, key_states, value_states):
        ...
        # will be ignored in future computation anyway
        # attention_mask = (1 - attention_mask.unsqueeze(1)) * -10000.  # (N, 1, Lq, L)
        mixed_query_layer = query_states
        mixed_key_layer = key_states
        mixed_value_layer = value_states
...
class ChangeDetectorDoubleAttDyn(nn.Module):
    ...
    def forward(self, input_1, input_2, change=False):
        ...
        input_bef = self.perl(input_bef, input_bef, input_bef)
        input_aft = self.perl(input_aft, input_aft, input_aft)

        bef_changeless = self.urd(input_bef, input_aft, input_aft)
        aft_changeless = self.urd(input_aft, input_bef, input_bef)

        bef_change = input_bef - bef_changeless
        aft_change = input_aft - aft_changeless

        input_diff = torch.cat([bef_change, aft_change], -1)
        input_diff = self.diff(input_diff)
        ...
        input_before = torch.cat([input_bef, input_diff], 1)
        input_after = torch.cat([input_aft, input_diff], 1)
        ...
        input_attended1 = attended_1 - attended_2
        input_attended2 = attended_2 - attended_1
